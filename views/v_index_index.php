<h2>Welcome to jDoku</h2>
<p>It's like sudoku, but with a "j." The rules are simple: each row, column, and 3x3 box must contain each of the numbers 1-9 exactly once.  Every puzzle has only one correct solution. Log in to track your puzzle solving stats, or just select a difficulty level and start solving now.  If you don't have an account, <a class="ajaxLink" href="/users/signup">sign up here</a>.</p>
<img src="/images/example_puzzle.png" id="examplePuzzle" />

<div id="newGame">
	<input type="button" class="submitBtn" value="Start Puzzle" id="startPuzzle" />
	<input type="button" class="difficultySelect" id="easy" value="Easy"></input>
	<input type="button" class="difficultySelect" id="medium" value="Medium"></input>
	<input type="button" class="difficultySelect" id="hard" value="Hard"></input>
	<input type="button" class="difficultySelect" id="vhard" value="Run Away Screaming"></input>
</div>